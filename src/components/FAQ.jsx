    ...
    <section className="py-24 border-t border-slate-200 overflow-hidden bg-background">
      <div className="mx-auto max-w-[800px] px-2.5 sm:px-6 lg:px-12">
        <div className="text-center max-w-2xl mx-auto mb-16 transition-all duration-700 opacity-100 translate-y-0 blur-0">
          <span className="text-sm font-medium text-amber-500 uppercase tracking-wider">FAQ</span>
          <h2 className="mt-3 text-3xl font-bold text-slate-900 md:text-4xl">Frequently asked questions</h2>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 p-6 lg:p-8 shadow-sm">
          {faqs.map((faq, idx) => (
            <div key={idx} className="border-b border-slate-200 transition-all duration-500 opacity-100 translate-x-0">
              <button
                className="w-full flex items-center justify-between py-5 text-left group"
                onClick={() => handleToggle(idx)}
                aria-expanded={openIndex === idx}
              >
                <span className="font-medium text-slate-800 group-hover:text-amber-500 transition-colors">{faq.question}</span>
                {openIndex === idx ? (
